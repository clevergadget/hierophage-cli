...
function formatMaintenanceEvent(event: TelemetryEvent): string {
  switch (event.type) {
    ...
    case 'weaver':
      return chalk.hex('#FFA500')(`Weaver: ${event.nodes.map(shortPath).join(' ~ ')} → ${event.recommendation} (${event.reason})`);
    default:
      return '';
  }
}
...
function getMaintenanceEventsInRange(events: TelemetryEvent[], startPulse: number, endPulse: number): TelemetryEvent[] {
  const maintenanceTypes = new Set(['scout', 'grazer', 'verifier_stability', 'verifier_coverage', 'propagation', 'resolver', 'synthesizer', 'weaver']);
...
function getNodesFromEvent(event: TelemetryEvent): string[] {
  switch (event.type) {
    ...
    case 'weaver': return event.nodes;
    default: return [];
  }
}
...
function renderAgentFilter(events: TelemetryEvent[], agentName: string): void {
  const matching = events.filter(e => {
    ...
    if (e.type === 'weaver') return agentName.toLowerCase() === 'weaver';
    return false;
  });

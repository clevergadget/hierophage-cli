...
describe('buildPrompt', () => {
  ...
  it('includes branch map when provided', () => {
    const target = makeNode('auth/login', 'Login');
    const branchMap = 'auth/\n  Login (auth/login)\napi/\n  Endpoints (api/endpoints)';

    const prompt = buildPrompt(target, '', [], undefined, branchMap);

    expect(prompt).toContain('## Branch Map (Scent Trails)');
    expect(prompt).toContain('auth/');
    expect(prompt).toContain('api/');
  });

  it('omits branch map section when not provided', () => {
    const target = makeNode('auth/login', 'Login');
    const prompt = buildPrompt(target, '', []);
    expect(prompt).not.toContain('Branch Map');
  });
});

describe('responseToMutations with overlaps', () => {
  it('parses response with overlaps field', () => {
    const target = makeNode('auth/login', 'Login', { content: 'Login form.' });
    const response = {
      reasoning: 'Found overlap',
      action: 'REVIEW',
      mutations: [],
      overlaps: [
        {
          target_path: 'auth/login',
          overlap_path: 'api/auth-endpoint',
          reason: 'Both handle authentication logic',
        },
      ],
    };

    // overlaps is passed through by FlashSpore, not by responseToMutations
    // So we just verify the response shape is valid
    expect(response.overlaps).toHaveLength(1);
    expect(response.overlaps[0].target_path).toBe('auth/login');
    expect(response.overlaps[0].overlap_path).toBe('api/auth-endpoint');
  });

  it('handles response without overlaps field', () => {
    const response = {
      reasoning: 'Normal pulse',
      action: 'DECOMPOSE',
      mutations: [],
    };

    expect(response).not.toHaveProperty('overlaps');
  });
});
